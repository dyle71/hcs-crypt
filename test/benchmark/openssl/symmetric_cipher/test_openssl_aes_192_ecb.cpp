...
TEST(Benchmark_OPENSSLAES192ECB, OPENSSLAES192ECBString) {

    auto loop_count = 100'000u;

    auto algo = headcode::crypt::Factory::Create("openssl-aes-192-ecb-encryptor");
    ASSERT_NE(algo.get(), nullptr);
    EXPECT_STREQ(algo->GetDescription().name_.c_str(), "openssl-aes-192-ecb-encryptor");
...
TEST(Benchmark_OPENSSLAES192ECB, OPENSSLAES192ECBCArray) {
    ...
    std::cout << StreamPerformanceIndicators(throughput,
                                             "Benchmark Benchmark_OPENSSLAES192ECB::OPENSSLAES192ECBCArray ");
}
