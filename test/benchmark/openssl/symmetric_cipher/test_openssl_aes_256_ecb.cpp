...
TEST(Benchmark_OPENSSLAES256ECB, OPENSSLAES256ECBString) {

    auto loop_count = 100'000u;

    auto algo = headcode::crypt::Factory::Create("openssl-aes-256-ecb-encryptor");
    ASSERT_NE(algo.get(), nullptr);
    EXPECT_STREQ(algo->GetDescription().name_.c_str(), "openssl-aes-256-ecb-encryptor");
    ...
    std::cout << StreamPerformanceIndicators(throughput,
                                             "Benchmark Benchmark_OPENSSLAES256ECB::OPENSSLAES256ECBString ");
}


TEST(Benchmark_OPENSSLAES256ECB, OPENSSLAES256ECBCArray) {
    ...
    std::cout << StreamPerformanceIndicators(throughput,
                                             "Benchmark Benchmark_OPENSSLAES256ECB::OPENSSLAES256ECBCArray ");
}
