...
TEST(Benchmark_LTCAES256ECB, LTCAES256ECBString) {

    auto loop_count = 100'000u;

    auto algo = headcode::crypt::Factory::Create("ltc-aes-256-ecb-encryptor");
    ASSERT_NE(algo.get(), nullptr);
    EXPECT_STREQ(algo->GetDescription().name_.c_str(), "ltc-aes-256-ecb-encryptor");
    ...
    std::cout << StreamPerformanceIndicators(throughput,
                                             "Benchmark Benchmark_LTCAES256ECB::LTCAES256ECBString ");
}


TEST(Benchmark_LTCAES256ECB, LTCAES256ECBCArray) {
    ...
    // makes rounds up the cipher size with a multiple of 256 bit
    std::vector<std::byte> cipher;
    ...
    std::cout << StreamPerformanceIndicators(throughput,
                                             "Benchmark Benchmark_LTCAES256ECB::LTCAES256ECBCArray ");
}
