...
def run(out_name: str, err_name: str) -> bool:
    ...
    out = open(out_name, 'w')
    err = open(err_name, 'w')
    p = subprocess.Popen([get_crypt_path(), '--version'], stdout=out, stderr=err)
...
def verify(out_name: str, err_name: str) -> bool:
    ...
    pattern = re.compile(r'crypt v\d\d*\.\d\d*\.\d\d*')
    line = out.readline()
    if pattern.match(line) is None:
        sys.stderr.write('Expected different pattern in result.\n')
        res = False
