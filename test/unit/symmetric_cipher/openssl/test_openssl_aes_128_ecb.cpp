...
TEST(SymmetricCipher_OpenSSL_AES_128_ECB, encryptor_creation) {
...
TEST(SymmetricCipher_OpenSSL_AES_128_ECB, decryptor_creation) {

    auto algo = headcode::crypt::Factory::Create("openssl-aes-128-ecb-decryptor");
    ASSERT_NE(algo.get(), nullptr);

    headcode::crypt::Algorithm::Description const & description = algo->GetDescription();

    EXPECT_STREQ(description.name_.c_str(), "openssl-aes-128-ecb-decryptor");
    EXPECT_EQ(description.family_, headcode::crypt::Family::kSymmetricCipher);
    EXPECT_FALSE(description.description_short_.empty());
    EXPECT_FALSE(description.description_long_.empty());
    EXPECT_EQ(description.block_size_incoming_, 16ul);
...
TEST(SymmetricCipher_OpenSSL_AES_128_ECB, single_block) {
    ...
    auto algo_dec = headcode::crypt::Factory::Create("openssl-aes-128-ecb-decryptor");
...
TEST(SymmetricCipher_OpenSSL_AES_128_ECB, regular) {
