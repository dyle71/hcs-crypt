...
TEST(SymmetricCipher_OpenSSL_AES_192_CBC, encryptor_creation) {

    auto algo = headcode::crypt::Factory::Create("openssl-aes-192-cbc-encryptor");
    ASSERT_NE(algo.get(), nullptr);

    headcode::crypt::Algorithm::Description const & description = algo->GetDescription();

    EXPECT_STREQ(description.name_.c_str(), "openssl-aes-192-cbc-encryptor");
...
TEST(SymmetricCipher_OpenSSL_AES_192_CBC, decryptor_creation) {

    auto algo = headcode::crypt::Factory::Create("openssl-aes-192-cbc-decryptor");
    ASSERT_NE(algo.get(), nullptr);

    headcode::crypt::Algorithm::Description const & description = algo->GetDescription();

    EXPECT_STREQ(description.name_.c_str(), "openssl-aes-192-cbc-decryptor");
    EXPECT_EQ(description.family_, headcode::crypt::Family::kSymmetricCipher);
    EXPECT_FALSE(description.description_short_.empty());
    EXPECT_FALSE(description.description_long_.empty());
    EXPECT_EQ(description.block_size_incoming_, 16ul);
...
TEST(SymmetricCipher_OpenSSL_AES_192_CBC, single_block) {
    ...
    auto algo_enc = headcode::crypt::Factory::Create("openssl-aes-192-cbc-encryptor");
    ...
    auto algo_dec = headcode::crypt::Factory::Create("openssl-aes-192-cbc-decryptor");
...
TEST(SymmetricCipher_OpenSSL_AES_192_CBC, regular) {
