...
TEST(SymmetricCipher_LTC_AES_256_ECB, encryptor_creation) {

    auto algo = headcode::crypt::Factory::Create("ltc-aes-256-ecb-encryptor");
    ASSERT_NE(algo.get(), nullptr);

    headcode::crypt::Algorithm::Description const & description = algo->GetDescription();

    EXPECT_STREQ(description.name_.c_str(), "ltc-aes-256-ecb-encryptor");
    ...
    EXPECT_EQ(description.initialization_argument_.size(), 1ul);
...
TEST(SymmetricCipher_LTC_AES_256_ECB, decryptor_creation) {

    auto algo = headcode::crypt::Factory::Create("ltc-aes-256-ecb-decryptor");
    ASSERT_NE(algo.get(), nullptr);

    headcode::crypt::Algorithm::Description const & description = algo->GetDescription();

    EXPECT_STREQ(description.name_.c_str(), "ltc-aes-256-ecb-decryptor");
    EXPECT_EQ(description.family_, headcode::crypt::Family::kSymmetricCipher);
    EXPECT_FALSE(description.description_short_.empty());
    EXPECT_FALSE(description.description_long_.empty());
    EXPECT_EQ(description.block_size_incoming_, 16ul);
...
TEST(SymmetricCipher_LTC_AES_256_ECB, single_block) {
    ...
    auto algo_enc = headcode::crypt::Factory::Create("ltc-aes-256-ecb-encryptor");
    ...
    ASSERT_EQ(algo_enc->Initialize({{"key", key_enc}}), 0);
    ...
    auto algo_dec = headcode::crypt::Factory::Create("ltc-aes-256-ecb-decryptor");
    ...
    ASSERT_EQ(algo_dec->Initialize({{"key", key_dec}}), 0);
    ...
    EXPECT_STREQ(headcode::mem::MemoryToHex(cipher).c_str(), "35dd7d6b893fe8ee9e3d12c7e2303a05");
}


TEST(SymmetricCipher_LTC_AES_256_ECB, regular) {
