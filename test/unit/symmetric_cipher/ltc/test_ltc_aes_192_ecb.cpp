...
TEST(SymmetricCipher_LTC_AES_192_ECB, encryptor_creation) {
    ...
    EXPECT_STREQ(description.name_.c_str(), "ltc-aes-192-ecb-encryptor");
...
TEST(SymmetricCipher_LTC_AES_192_ECB, decryptor_creation) {

    auto algo = headcode::crypt::Factory::Create("ltc-aes-192-ecb-decryptor");
    ASSERT_NE(algo.get(), nullptr);

    headcode::crypt::Algorithm::Description const & description = algo->GetDescription();

    EXPECT_STREQ(description.name_.c_str(), "ltc-aes-192-ecb-decryptor");
    EXPECT_EQ(description.family_, headcode::crypt::Family::kSymmetricCipher);
    EXPECT_FALSE(description.description_short_.empty());
    EXPECT_FALSE(description.description_long_.empty());
    EXPECT_EQ(description.block_size_incoming_, 16ul);
...
TEST(SymmetricCipher_LTC_AES_192_ECB, single_block) {
    ...
    auto algo_enc = headcode::crypt::Factory::Create("ltc-aes-192-ecb-encryptor");
    ...
    auto algo_dec = headcode::crypt::Factory::Create("ltc-aes-192-ecb-decryptor");
...
TEST(SymmetricCipher_LTC_AES_192_ECB, regular) {
