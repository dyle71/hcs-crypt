...
TEST(Hash_OPENSSLSHA512, creation) {
    ...
    EXPECT_STREQ(description.name_.c_str(), "openssl-sha512");
...
TEST(Hash_OPENSSLSHA512, simple) {
...
TEST(Hash_OPENSSLSHA512, regular) {

    auto algo = headcode::crypt::Factory::Create("openssl-sha512");
    ASSERT_NE(algo.get(), nullptr);
    ASSERT_STREQ(algo->GetDescription().name_.c_str(), "openssl-sha512");
    EXPECT_EQ(algo->Initialize(), 0);
...
TEST(Hash_OPENSSLSHA512, noinit) {
