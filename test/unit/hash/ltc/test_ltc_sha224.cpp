...
TEST(Hash_LTCSHA224, creation) {
    ...
    EXPECT_STREQ(description.name_.c_str(), "ltc-sha224");
    ...
    EXPECT_EQ(description.block_size_outgoing_, 28ul);
...
TEST(Hash_LTCSHA224, regular) {

    auto algo = headcode::crypt::Factory::Create("ltc-sha224");
    ASSERT_NE(algo.get(), nullptr);
    ASSERT_STREQ(algo->GetDescription().name_.c_str(), "ltc-sha224");
    EXPECT_EQ(algo->Initialize(), 0);
