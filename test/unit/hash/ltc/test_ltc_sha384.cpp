...
TEST(Hash_LTCSHA384, creation) {
    ...
    EXPECT_STREQ(description.name_.c_str(), "ltc-sha384");
...
TEST(Hash_LTCSHA384, regular) {

    auto algo = headcode::crypt::Factory::Create("ltc-sha384");
    ASSERT_NE(algo.get(), nullptr);
    ASSERT_STREQ(algo->GetDescription().name_.c_str(), "ltc-sha384");
    EXPECT_EQ(algo->Initialize(), 0);
