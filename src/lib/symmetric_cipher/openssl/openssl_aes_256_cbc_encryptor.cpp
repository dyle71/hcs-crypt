...
#include "openssl_aes_256_cbc_encryptor.hpp"

using namespace headcode::crypt;


/**
 * @brief   The OpenSSL AES 256 CBC algorithm (encryptor) description.
 ...
 */
class OpenSSLAES256CBCEncrypterProducer : public Factory::Producer {
public:
    /**
    ...
    std::unique_ptr<Algorithm> operator()() const override {
        return std::make_unique<OpenSSLAES256CBCEncrypter>();
    }
...
EVP_CIPHER const * OpenSSLAES256CBCEncrypter::GetCipher() const {
    return EVP_aes_256_cbc();
}


Algorithm::Description const & OpenSSLAES256CBCEncrypter::GetDescription_() const {
    return ::GetDescription();
}


void OpenSSLAES256CBCEncrypter::Register() {
    auto const & description = ::GetDescription();
    Factory::Register(description.name_, description.family_, std::make_shared<OpenSSLAES256CBCEncrypterProducer>());
}
